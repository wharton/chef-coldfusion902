...
directory "#{Chef::Config['file_cache_path']}/APSB13-10" do
  action :create
  not_if { File.exists?("#{node['cf902']['install_path']}/lib/updates/hf902-00004.jar") }
end

remote_file "#{Chef::Config['file_cache_path']}/APSB13-10/CF902.zip" do
  source "#{node['cf902']['APSB13-10']['CF902']['url']}"
  action :create
  mode "0744"
  owner "root"
  group "root"
...
remote_file "#{Chef::Config['file_cache_path']}/APSB13-10/CFIDE-CF902.zip" do
  source "#{node['cf902']['APSB13-10']['CFIDE-CF902']['url']}"
  action :create
  mode "0744"
  owner "root"
  group "root"
...
script "install_APSB13-10" do
  interpreter "bash"
  user "root"
  cwd "#{Chef::Config['file_cache_path']}/APSB13-10"
  code <<-EOH
  unzip CF902.zip
  cp CF902/lib/updates/hf902-00004.jar #{node['cf902']['install_path']}/lib/updates   
  unzip -o CFIDE-902.zip -d #{node['cf902']['install_path']}/wwwroot
  chown -R nobody:bin #{node['cf902']['install_path']}/wwwroot
  chown -R nobody:bin #{node['cf902']['install_path']}/lib
  rm -fR CF902
